...
export const RecoveryStreak = ({selected ,dayNumber,achieveType,setSelected}:IRecoveryStreakrProps ) => {
    const overlay = useOverlay();
    const commonClass ="flex text-white text-2xl w-16 h-16 rounded-md items-center justify-center"
    const DISABLED = "flex text-white text-2xl w-16 h-16 rounded-md items-center justify-center"
    ...
    const buttonClick = () => {
        if(achieveType === 'NOT_ACHIEVE'){
            ...
            overlay.open(({ isOpen, close }) => (
                <AlertBox label={'스트릭을 복구할 수 없는 날짜입니다'} mode="WARNING" open={isOpen} />
            ));
    ...
    return(
        <>
            <motion.button
                ...
                animate={{
                    backgroundColor: achieveType=='NOT_ACHIEVE' ? selected ? '#15481b' : '#acd4be' :'#c9c9c9',
